...
def HarrisDetector(src, gaus_filter_size = 3, gaus_sigma = 1, alpha = 0.04, threshold_rate = 0.01):
    ...
    """
    har = (G_IxIx * G_IyIy) - (G_IxIy * G_IxIy) - alpha * (G_IxIx + G_IyIy)**2
...
def main():
    ...
    cv2.imwrite('interest points.png', interest_points)
    cv2.imshow('harris_img', harris_img)
    cv2.imwrite('harris_img.png', harris_img)
    cv2.waitKey()
