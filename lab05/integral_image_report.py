...
def get_integral_image(src):
    ...
    for row in range(h):
        for col in range(w):
            if row == 0 or col == 0 :
                dst[row, col] = np.sum(src[:row+1, :col+1])
            else:
                dst[row, col] = dst[row-1, col] - dst[row-1, col-1] + dst[row, col-1] + src[row, col]
    return dst
...
def harris_detector_integral(src, k = 0.04, threshold_rate = 0.01, fsize=5):
    ...
    for row in range(h-(fsize-1)):
        for col in range(w-(fsize-1)):
            if row == 0 and col == 0:
                M_integral[row, col, 0, 0] = IxIx_integral[row + (fsize-1), col + (fsize-1)]
                M_integral[row, col, 0, 1] = IxIy_integral[row + (fsize-1), col + (fsize-1)]
                ...
                M_integral[row, col, 1, 0] = M_integral[row, col, 0, 1]
                M_integral[row, col, 1, 0] = IyIy_integral[row - 1, col-1] + IyIy_integral[row +(fsize-1), col +(fsize-1)] - IyIy_integral[row-1, col +(fsize-1)] - IyIy_integral[row+(fsize-1), col-1]
...
def main():
    ...
    cv2.imshow('harris_img ' + '201802161' , harris_img)
    cv2.imshow('harris_integral_img ' + '201802161' , harris_integral_img)
    cv2.waitKey()
