...
def feature_matching(img1, img2, RANSAC=False, threshold = 300, keypoint_num = None, iter_num = 500, threshold_distance = 10):
    ...
    if not RANSAC:
        ...
        # Backward 방식
        h, w = img1.shape[:2]
        dst = np.zeros((int(np.round(h*2)), int(np.round(w*2)), 3), dtype=np.uint8)
        h_, w_ = dst.shape[:2]
        ...
        for row in range(h_):
            for col in range(w_):
                ...
                s = c - c_left
                t = r - r_top
                if r_bottom >= h - 1 or c_right >= w - 1 or c_left < 0 or r_top < 0:
                    dst[row, col] = 0
                else:
    ...
    # use RANSAAC
        ...
        for i in range(iter_num):
            ...
            for idx, point in enumerate(points):
                ...
                '''
                # ???(M으로 구한 point)# point1에 M을 곱해서 구한거 말하는듯
                # ???(실제 point) # point안에 img1이랑 img2 각각의 좌표 있는거 그거 말하는듯
                M_point = np.dot(M, [[point[0][0]], [point[0][1]], [1]])
                real_point = [point[1][0], point[1][1]]
                if L2_distance(np.array([M_point[0][0], M_point[1][0]]), np.array(real_point)) < threshold_distance:
                    count_inliers += 1
...
def main():
    ...
    ###딱 한 픽셀에만 점을 찍기 위해 소수의 경우 가장 가까운 위치로 변경 ex : (1.9, 1.8)이 row_와 col_으로 나온 경우 row : 2, col : 2로 변경
    '''
    vec = np.dot(M_FM_RANSAC, [[160],[160],[1]])
    col_ = int(np.round(vec[0,0]))
    row_ = int(np.round(vec[1,0]))
    ...
    cv2.imshow('img_point 201802161', img_point)
    cv2.imshow('dst 201802161', dst)
    cv2.imshow('dst_FM 201802161', dst_FM)
    cv2.imshow('dst_FM_RANSAC 201802161', dst_FM_RANSAC)
